...
import (
	...
	"github.com/ryo0210/go_todo_app/testutil/fixture"
)
...
func TestEmbed(t *testing.T) {
	...
	fmt.Printf("want %s, but got %s", want, rawPubKey)
...
func TestJWTer_GetJWT(t *testing.T) {
	...
	want, err := jwt.NewBuilder().
		...
		Issuer(`github.com/ryo0210/go_todo_app`).
	...
	req := httptest.NewRequest(
		http.MethodGet,
		`https://github.com/ryo0210`,
		nil,
	)
	req.Header.Set(`Authorization`, fmt.Sprintf(`Bearer %s`, signed))
	got, err := sut.GetToken(ctx, req)
	if err != nil {
		t.Fatalf("want no error, but go %v", err)
	}
